...
CREATE TABLE pms_board (
  ...
  created_dt DATETIME     NULL     DEFAULT now() COMMENT '등록일', -- 등록일
...
CREATE TABLE pms_task (
  ...
  project_no     INTEGER NULL     COMMENT '프로젝트번호', -- 프로젝트번호
  member_no      INTEGER NULL     COMMENT '회원번호', -- 회원번호
  contents       TEXT    NULL     COMMENT '내용', -- 내용
  deadline       DATE    NULL     COMMENT '마감일', -- 마감일
...
ALTER TABLE pms_like
  ADD CONSTRAINT PK_pms_like -- 즐겨찾기 기본키
    PRIMARY KEY (
      board_no,  -- 게시글번호
      member_no  -- 회원번호
    );
...
CREATE TABLE pms_task_status (
  task_status_no INTEGER     NOT NULL COMMENT '작업상태번호', -- 작업상태번호
  title          VARCHAR(80) NULL     COMMENT '상태명' -- 상태명
)
COMMENT '작업상태';
...
-- 즐겨찾기
ALTER TABLE pms_like
  ADD CONSTRAINT FK_pms_board_TO_pms_like -- 게시판 -> 즐겨찾기
    FOREIGN KEY (
      board_no -- 게시글번호
    )
    REFERENCES pms_board ( -- 게시판
...
-- 즐겨찾기
ALTER TABLE pms_like
  ADD CONSTRAINT FK_pms_member_TO_pms_like -- 회원 -> 즐겨찾기
    FOREIGN KEY (
      member_no -- 회원번호
    )
    REFERENCES pms_member ( -- 회원
